...
if (process.platform === "darwin") {
  connTray = path.join(__dirname, "img", "tray_connected_osxTemplate.png")
  disconnTray = path.join(__dirname, "img", "tray_disconnected_osxTemplate.png")
} else if (process.platform === "win32") {
  connTray = path.join(__dirname, "img", "tray_connected_win.png")
  disconnTray = path.join(__dirname, "img", "tray_disconnected_win.png")
} else if (process.platform === "linux") {
  connTray = path.join(__dirname, "img", "tray_connected_linux_light.png")
  disconnTray = path.join(__dirname, "img", "tray_disconnected_linux_light.png")
} else {
  connTray = path.join(__dirname, "img", "tray_connected.png")
  disconnTray = path.join(__dirname, "img", "tray_disconnected.png")
}
    ...
    function(err, resp, body) {
      ...
      try {
        ...
        err = new errors.ParseError(
          "main: Failed to parse service status (%s)",
          e
        )
        logger.error(err)
        if (tray) {
      ...
      events.subscribe(function(evt) {
        if (evt.type === "output") {
          ...
          fs.appendFile(pth, evt.data.output + "\n", function(err) {
            if (err) {
              err = new errors.ParseError(
                "main: Failed to append profile output (%s)",
                err
              )
              logger.error(err)
            }
