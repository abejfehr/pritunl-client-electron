...
// var colors = ["#D32F2F", "#2196F3", "#795548", "#5C6BC0", "#66BB6A"]
    ...
    prfl.postConnect(true, function(authType, callback) {
      ...
      var authHandler = function() {
        if (authType.indexOf("username") !== -1) {
          ...
          $profile
            .find(".connect-otp-input")
            .attr("placeholder", "Enter OTP Code")
...
var refreshProfiles = function() {
  profile.getProfiles(function(err, prfls) {
    if (err) {
      err = new errors.ReadError("importer: Failed to read profiles (%s)", err)
      logger.error(err)
      return
    }
