    ...
    function(err) {
      if (err) {
        err = new errors.WriteError(
          "config: Failed to save profile conf (%s)",
          err
        )
        logger.error(err)
      }
...
Profile.prototype.saveData = function(callback) {
  ...
  fs.writeFile(
    ...
    function(err) {
      if (err) {
        err = new errors.WriteError(
          "config: Failed to save profile data (%s)",
          err
        )
        logger.error(err)
      }
...
Profile.prototype.saveLog = function(callback) {
  fs.writeFile(this.logPath, this.log, function(err) {
    if (err) {
      err = new errors.WriteError(
        "config: Failed to save profile log (%s)",
        err
      )
      logger.error(err)
    }
...
Profile.prototype.delete = function() {
  ...
  if (os.platform() === "darwin") {
    childProcess.exec(
      "/usr/bin/security delete-generic-password -s pritunl -a " + this.id,
      function() {}.bind(this)
    )
  }
      ...
      fs.unlink(this.confPath, function(err) {
        if (err) {
          err = new errors.RemoveError(
            "config: Failed to delete profile conf (%s)",
            err
          )
          logger.error(err)
        }
  ...
  fs.exists(
    ...
    function(exists) {
      ...
      fs.unlink(this.ovpnPath, function(err) {
        if (err) {
          err = new errors.RemoveError(
            "config: Failed to delete profile data (%s)",
            err
          )
          logger.error(err)
        }
  ...
  fs.exists(
    ...
    function(exists) {
      ...
      fs.unlink(this.logPath, function(err) {
        if (err) {
          err = new errors.RemoveError(
            "config: Failed to delete profile log (%s)",
            err
          )
          logger.error(err)
        }
...
Profile.prototype.extractKey = function() {
  ...
  if (sIndex > 0 && eIndex > 0) {
    ...
    this.data =
      this.data.substring(0, sIndex) +
      this.data.substring(eIndex + 12, this.data.length)
  }
  ...
  if (sIndex > 0 && eIndex > 0) {
    ...
    this.data =
      this.data.substring(0, sIndex) +
      this.data.substring(eIndex + 7, this.data.length)
  }
          ...
          function(err, stdout, stderr) {
            if (err) {
              err = new errors.ProcessError(
                "profile: Failed to add key to keychain (%s)",
                stderr
              )
              logger.error(err)
            }
...
Profile.prototype.getFullData = function(callback) {
  ...
  childProcess.exec(
    "/usr/bin/security find-generic-password -w -s pritunl -a " + this.id,
    function(err, stdout, stderr) {
      if (err) {
        err = new errors.ProcessError(
          "profile: Failed to get key from keychain (%s)",
          stderr
        )
        logger.error(err)
        return
...
Profile.prototype.updateSync = function(data) {
  ...
  if (
    this.data.indexOf("key-direction") >= 0 &&
    data.indexOf("key-direction") < 0
  ) {
  ...
  utils.authRequest(
    ...
    function(err, resp, body) {
      if (resp && resp.statusCode === 480) {
        ...
        logger.warning(
          "profile: Failed to sync conf, " + "authentication error"
        )
        ...
        if (body) {
          ...
          var confSignature = crypto
            .createHmac("sha512", this.syncSecret)
            .update(data.conf)
            .digest("base64")
        ...
        if (!syncHosts.length) {
          if (resp) {
            logger.warning(
              "profile: Failed to sync config (" + resp.statusCode + ")"
            )
          } else {
...
Profile.prototype._auth = function(authType, timeout, callback) {
  if (!authType) {
    ...
    service.start(this, timeout, this.serverPublicKey, this.serverBoxPublicKey)
  } else if (!callback) {
  } else {
    callback(
      authType,
